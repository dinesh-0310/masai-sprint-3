...
window.addEventListener('load',function(){
    ...
    idLeague = localStorage.getItem('idLeague')
...
function rendorDOM(){
    ...
    var url = "https://www.thesportsdb.com/api/v1/json/1/lookupleague.php?id="
    xhr.open('GET', url + idLeague)
    xhr.send()
    xhr.onload = function(){
        var res = JSON.parse(xhr.response)
        // console.log(res.leagues);
        displayData(res.leagues)
...
function displayData(data){
    ...
    banner_img.setAttribute('src', data[0].strBanner)
    var h1 = document.createElement('h1')
    h1.textContent = `About ${data[0].strLeague}`
    ...
    table_info.innerHTML = `<table border="1">
                        <tbody>
                            ...
                            <tr> <td>First Event</td><td>${data[0].dateFirstEvent}</td>
                            </tr>
                            <tr> <td>Country</td><td>${data[0].strCountry}</td>
                            </tr>
                            <tr> <td>Trophy</td><td><img src = "${data[0].strTrophy}" alt="Trophy"></td>
                            </tr>
                            <tr> <td>Website</td><td><a href = "${data[0].strWebsite}">${data[0].strLeague}</td>
                            </tr>
                        </tbody>
                    </table>`
